    ...
    def do_GET(self):
        ...
        if parsed.path.startswith('/api/'):
            if parsed.path == '/api/ai-decision':
                ...
                # Return forex prices (crypto prices come from frontend WebSocket)
                combined = {**forex_market_state.get("prices", {})}
                ...
                # Crypto history is handled by frontend directly via Binance API
                self.send_json_response([], status=404)
            else:
        ...
        if parsed.path == '/api/ai-decision':
            ...
            try:
                ...
                with state_lock:
                    ...
                    # Update other fields
                    skip_keys = [
                        "balance_spot", "balance_futures", "balance_forex",
                        "pnl_spot", "pnl_futures", "pnl_forex",
                        "positions" 
                    ]
